...
# STREAMLIT UI
# -----------------------------------------------------------------------------
st.set_page_config(page_title="Content Moderation (Naive Bayes)", layout="wide",
                   initial_sidebar_state="expanded")

# SIDEBAR
st.sidebar.title("Deployment & Settings")
st.sidebar.markdown("Upload model files in the repo root or provide raw GitHub URLs.")
model_path = st.sidebar.text_input("Local model path", value=DEFAULT_MODEL_PATH)
vect_path = st.sidebar.text_input("Local vectorizer path", value=DEFAULT_VECT_PATH)
model_url = st.sidebar.text_input("Model raw GitHub URL (optional)", value="")
vect_url = st.sidebar.text_input("Vectorizer raw GitHub URL (optional)", value="")

st.sidebar.subheader("Thresholds (per-label)")
default_thresholds = {c: 0.5 for c in TARGET_COLS}
thresholds = {}
for col in TARGET_COLS:
    thresholds[col] = st.sidebar.slider(col, 0.01, 0.99, 0.5, 0.01)

st.sidebar.markdown("---")
st.sidebar.markdown("**Batch upload:** CSV with columns `id` and `comment_text`")
st.sidebar.markdown("**Output:** `submission.csv` (id + target columns)")

# main
st.title("🚦 Content Moderation — Naive Bayes Deployment")
st.markdown(
    """
    Professional, simple interface for your trained Naive Bayes content moderation model.
    *Supports single-text prediction and batch CSV predictions (downloadable submission).*
    """
)
...
st.markdown("---")
st.subheader("Sample submission (if provided in repo)")
try:
    sample = pd.read_csv(SAMPLE_SUB_PATH)
    st.write("Sample submission preview:")
    st.dataframe(sample.head())
except Exception:
    st.info("No sample_submission.csv found in repo (optional).")

# Footer: quick tips
st.markdown("""
---
**Notes**
- If you deploy this app on Streamlit Cloud, put `naive_bayes_model.pkl` and `tfidf_vectorizer.pkl` in the repository root.
- If model files are large, use Git LFS (instructions later).
- Adjust thresholds in the sidebar to trade off precision/recall per-label.
""")
